...
def handle_user_feedback(audience_json: Dict[str, Any]) -> None:
    ...
    if render_button("Apply Feedback"):
        with st.spinner("Applying feedback..."):
            state.old_audience_json = audience_json
...
def process_and_render_segments() -> None:
    ...
    with st.spinner("Processing audience segments..."):
        if not state.summary_results:
            summary_results = process_audience_data(ensure_dict(state.extracted_audience_json))
...
def generate_methodology_report() -> None:
    ...
    with st.spinner("Generating data collection methodology summaries..."):
        segments = extract_research_inputs(state.summary_results)
...
def main() -> None:
    ...
    if render_button("Generate Audience"):
        ...
        generate_initial_audience(state.company_name, state.conversation_history)

    if state.stage >= 1:
        audience_json = ensure_dict(state.extracted_audience_json)
        
        handle_segment_selection(audience_json)
        
        if state.old_audience_json:
            render_json_diff(state.old_audience_json, audience_json)
